...
#| chaff (tdelvecc, Aug 29, 2020): 
    Raises an error when recommend provided a title not in any files.
...
fun recommend(title :: String, book-records :: List<File>) 
  ...
  when not(book-records.any({(record): record.content.member(title)})):
    raise("Title missing from files.")
  end
