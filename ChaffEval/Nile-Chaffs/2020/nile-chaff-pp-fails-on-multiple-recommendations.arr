...
#| chaff (tdelvecc, Aug 29, 2020): 
    Raises an error when resulting recommendation has multiple pairs in popular pairs.
    Search "CHAFF DIFFERENCE" for changed code.
|#

include shared-gdrive("nile-support.arr", "1IWAMLoV3B_WzDM0l9oNCW6GB1cgaqnYR")
...
fun popular-pairs(book-records :: List<File>) -> Recommendation<BookPair>:
  ...
  # CHAFF DIFFERENCE: Raises an error when recommendation has multiple pairs.
  if result.count > 1:
    raise("Result has multiple pairs.")
  else:
    result
  end
end
