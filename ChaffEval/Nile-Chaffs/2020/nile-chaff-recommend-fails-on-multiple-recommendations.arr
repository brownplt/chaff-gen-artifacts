...
#| chaff (tdelvecc, Aug 29, 2020): 
    Raises an error when resulting recommendation has multiple books in recommend.
...
fun recommend(title :: String, book-records :: List<File>) 
  ...
  # CHAFF DIFFERENCE: Raises an error when recommendation has multiple books.
  if result.count > 1:
    raise("Result has multiple books.")
  else:
    result
  end
end
