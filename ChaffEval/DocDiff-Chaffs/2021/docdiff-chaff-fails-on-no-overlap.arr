...
#| chaff (tdelvecc, Aug 26, 2020): 
    Raises an error when the two docs have no overlap.
...
fun overlap(doc1 :: List<String>%(is-link), doc2 :: List<String>%(is-link)) -> Number block:
  ...
  # CHAFF DIFFERENCE: Raises an error when docs have no overlap.
  when dot-product(vec1, vec2) == 0:
    raise("No overlap in docs.")
  end
