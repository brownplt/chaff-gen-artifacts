...
# END HEADER
#| 
    Does not obey the following subproperty: 
        Consumes a Dir and produces the number of files in the directory tree.
    Instead: Counts files AND directories in the current tree
|#
...
fun how-many(directory :: Dir) -> Number:
  ...
  for fold(num-files from directory.fs.length() + directory.ds.length(), sub-dir from directory.ds):
