...
#| chaff (tdelvecc, Aug 31, 2020): 
    Fynd also returns paths to directories with the given name.
...
fun fynd(directory :: Dir, name :: String) -> List<Path>:
  ...
  # CHAFF DIFFERENCE: Also adds directory if a sub-directory has given name.
  if directory.fs.map(_.name).member(name) or directory.ds.map(_.name).member(name):
