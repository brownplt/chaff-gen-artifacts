...
#| wheat (tdelvecc, Aug 26, 2020): 
   Does NOT illustrate the following property:
    ...
    Instead: Squares the squared magnitude of the larger vector (dot product w/ itself squared)
|#
...
fun overlap(doc1 :: List<String>%(is-link), doc2 :: List<String>%(is-link)) -> Number:
  ...
  dot-product(vec1, vec2) / num-sqr(num-max( dot-product(vec1, vec1), dot-product(vec2, vec2)))
  #|
where:
