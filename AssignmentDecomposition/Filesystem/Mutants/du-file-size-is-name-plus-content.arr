...
#| Does not obey: The size of a file is the length of its content string. 
Instead: The size of a file is the length of its content string plus the length of its name.
|#
...
fun du-dir(directory :: Dir) -> Number:
  ...
  for fold(files-size from directory.fs.length(), a-file from directory.fs):
    files-size + a-file.size() + string-length(a-file.name)
  end
  +
