...
provide {how-many: how-many, du-dir: du-dir, can-find: can-find, fynd: fynd-wrapper} end

#| 

    Does not obey: Fynd: Should not identify directories as files.
    Instead: Identifies directories as files.
|#
...
fun get-subdirs(directory :: Dir, directory_path :: List<Path>) -> List<Path>:

    subdir = directory.ds.map( lam(d): directory_path.append([list: d.name]) end)
    children = directory.ds.map( 
        lam(d): 
            dp = directory_path.append([list: d.name]) 
            get-subdirs(d, dp)   end)

    children_flattened = lists.foldl(lists.append, empty, children)
    subdir.append(children_flattened)
end

fun fynd-dirs(directory :: Dir, name :: String) -> List<Path>:
  doc: "Finds all instances of dirs with given name in the directory tree."
  sub-dirs = get-subdirs(directory, [list: directory.name])


  sub-dirs.filter( lam(x): x.last() == name end)
           .map(lam(x): x.take(x.length() - 1) end )
end
...
fun fynd(directory :: Dir, name :: String) -> List<Path>:
  ...
  # If file or directory is in current directory, add new path
...
fun fynd-wrapper(directory :: Dir, name :: String) -> List<Path>:

    fynd(directory, name).append(fynd-dirs(directory, name))
