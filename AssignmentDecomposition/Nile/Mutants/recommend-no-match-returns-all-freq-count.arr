...
#| Does not obey: When no pairing can be found, recommend should return recommendation(0,[list: ]).
    Instead: returns all books with freq  = length.
...
fun recommend(title :: String, book-records :: List<File>) 
  ...
  if result.count < 1:
     recommendation(get-all-books(book-records).length(), get-all-books(book-records))
  else:
    result
  end
end
