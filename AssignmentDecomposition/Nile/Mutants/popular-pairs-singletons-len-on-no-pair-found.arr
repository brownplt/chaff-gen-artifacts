...
#| 
    ...
    Instead: Returns a list of all books with no paired book, with freq = # of books.
|#
...
fun popular-pairs(book-records :: List<File>) -> Recommendation<BookPair>:
  ...
  if result == recommendation(0, empty):
    recommendation((get-all-books(book-records)).length(), (get-all-books(book-records)).map(lam(book): pair(book, '') end))
  else:
    result
  end
end