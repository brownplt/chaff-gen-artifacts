...
fun recommend(title :: String, book-records :: List<File>) 
  ...
  result = book-records
    ...
    in-multiple-files = result.content.filter(

    lam(book):  
    
      present = book-records.filter(lam(record): record.content.member(book) and record.content.member(title) end)

      present.length() > 1
    end)
  
  if in-multiple-files.length() == 0:
    recommendation(0, empty)
  else:
    recommendation(result.count, in-multiple-files)
  end
end
